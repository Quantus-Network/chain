...
//                 (ConnectionState::OpeningThenClosing, ConnectionState::OpeningThenClosing) =>
// true,                 (ConnectionState::OpenDesiredByRemote,
// ConnectionState::OpenDesiredByRemote) => {                     true
//                 }
...
//         if let Some(&PeerState::Disabled {
//             ref connections,
//             backoff_until: None,
//         }) = notif.peers.get(&(peer, 0.into()))
//         {
...
//         notif.on_swarm_event(FromSwarm::DialFailure(
...
//         assert_eq!(
//             conn_yielder.get_next_event(peer, set_id.into()).await,
//             Some(vec![1, 3, 3, 7])
//         );
//     }
...
//         // manually add backoff for the entry
//         if let Some(&mut PeerState::Incoming {
//             ref mut backoff_until,
//             ..
...
//         if let Some(PeerState::PendingRequest {
//             ref timer_deadline, ..
//         }) = notif.peers.get(&(peer, set_id))
//         {
...
//             assert_eq!(
//                 connections[0],
//                 (conn1, ConnectionState::OpenDesiredByRemote)
//             );
//             assert_eq!(
//                 connections[1],
//                 (conn2, ConnectionState::OpenDesiredByRemote)
//             );
//         }
//     }
...
//         if let Some(&mut PeerState::Disabled {
//             ref mut backoff_until,
//             ..
//         }) = notif.peers.get_mut(&(peer, 0.into()))
//         {
...
//         let prev_instant = if let Some(PeerState::DisabledPendingEnable {
//             ref connections,
//             timer_deadline,
//             ..
//         }) = notif.peers.get(&(peer, set_id))
...
//             .incoming
//             .iter()
//             .any(|entry| entry.incoming_id == IncomingIndex(0)));
//         notif.disconnect_peer(&peer, set_id);
