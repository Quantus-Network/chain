...
mod tests {
    ...
    async fn basic_works() {
        ...
        let client = tokio::spawn(async move {
            let (handshake, mut substream) =
                dial(listener_addr_rx.await.unwrap(), &b"initial message"[..])
                    .await
                    .unwrap();
    ...
    async fn send_handshake_without_polling_for_incoming_data() {
        ...
        future::poll_fn(|cx| Pin::new(&mut substream).poll_process(cx))
            .await
            .unwrap();
