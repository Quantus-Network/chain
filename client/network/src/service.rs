    ...
    H: ExHashT,
{
    ...
    fn peer_store(
        ...
        PeerStore::new(
            bootnodes.into_iter().map(From::from).collect(),
            metrics_registry,
        )
    }
    ...
    H: ExHashT,
{
    ...
    pub fn new(params: Params<B, H, Self>) -> Result<Self, Error> {
        ...
        let all_peer_sets_iter = iter::once(&network_config.default_peers_set).chain(
            notification_protocols
                .iter()
                .map(|protocol| protocol.set_config()),
        );
...
impl<B: BlockT + 'static, H: ExHashT> NetworkService<B, H> {
    ...
    pub async fn network_state(&self) -> Result<NetworkState, ()> {
        ...
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::NetworkState {
                pending_response: tx,
            });
    ...
    H: ExHashT,
{
    ...
    fn put_record_to(
        ...
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::PutRecordTo {
                record,
                peers,
    ...
    fn store_record(
        ...
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::StoreRecord(
                key,
                value,
    ...
    H: ExHashT,
{
    async fn status(&self) -> Result<NetworkStatus, ()> {
        ...
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::NetworkStatus {
                pending_response: tx,
            });
    ...
    H: ExHashT,
{
    ...
    fn add_known_address(
        ...
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::AddKnownAddress(
                peer_id.into(),
                addr.into(),
            ));
    }
    ...
    fn add_reserved_peer(&self, peer: MultiaddrWithPeerId) -> Result<(), String> {
        ...
        self.sync_protocol_handle
            .add_reserved_peer(peer.peer_id.into());
    ...
    fn remove_reserved_peer(&self, peer_id: sc_network_types::PeerId) {
        self.sync_protocol_handle
            .remove_reserved_peer(peer_id.into());
    }
    ...
    fn set_reserved_peers(
        ...
        let Some(set_id) = self.notification_protocol_ids.get(&protocol) else {
            return Err(format!(
                "Cannot set reserved peers for unknown protocol: {}",
                protocol
            ));
        };
    ...
    H: ExHashT,
{
    ...
    pub async fn next_action(&mut self) -> bool {
        ...
        let num_connected_peers = self
            ...
            .num_sync_peers();
        self.num_connected
            .store(num_connected_peers, Ordering::Relaxed);
        ...
        if let Some(metrics) = self.metrics.as_ref() {
            ...
            metrics.pending_connections.set(
                Swarm::network_info(&self.network_service)
                    .connection_counters()
                    .num_pending() as u64,
            );
        }
    ...
    fn handle_worker_message(&mut self, msg: ServiceToWorkerMsg) {
        match msg {
            ...
            ServiceToWorkerMsg::PutValue(key, value) => self
                .network_service
                .behaviour_mut()
                .put_value(key.into(), value),
            ServiceToWorkerMsg::PutRecordTo {
                record,
                peers,
                update_local_storage,
            } => self.network_service.behaviour_mut().put_record_to(
                record.into(),
                peers,
                update_local_storage,
            ),
            ...
            ServiceToWorkerMsg::AddKnownAddress(peer_id, addr) => self
                .network_service
                .behaviour_mut()
                .add_known_address(peer_id, addr),
        ...
        match event {
            ...
            SwarmEvent::Behaviour(BehaviourOut::PeerIdentify {
                ...
                for addr in listen_addrs {
                    self.network_service
                        .behaviour_mut()
                        .add_self_reported_address_to_dht(&peer_id, &protocols, addr.clone());
                }
            ...
            SwarmEvent::OutgoingConnectionError {
                ...
                if let Some(peer_id) = peer_id {
                    ...
                    if let Some(addresses) = not_reported
                        .then(|| self.boot_node_ids.get(&peer_id))
                        .flatten()
                    {
            ...
            SwarmEvent::ListenerClosed {
                ...
                if let Some(metrics) = self.metrics.as_ref() {
                    metrics
                        .listeners_local_addresses
                        .sub(addresses.len() as u64);
                }
