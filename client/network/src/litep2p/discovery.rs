...
/// The minimum number of peers we expect to store the record before we consider the put successful.
const PUT_RECORD_REDUNDANCY_FACTOR: usize = 4;
	...
	pub async fn put_value(&mut self, key: KademliaKey, value: Vec<u8>) -> QueryId {
		self.kademlia_handle
			.put_record(
				...
				Quorum::N(NonZeroUsize::new(PUT_RECORD_REDUNDANCY_FACTOR).unwrap()),
			)
			.await
	}
	...
	fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
		...
		match Pin::new(&mut this.kademlia_handle).poll_next(cx) {
			...
			Poll::Ready(Some(KademliaEvent::AddProviderSuccess { query_id, provided_key })) => {
				log::trace!(
					target: LOG_TARGET,
					"`ADD_PROVIDER` succeeded for {query_id:?}, key: {provided_key:?}",
				);
				// We don't emit a specific event for this, just log it
			},
		}
