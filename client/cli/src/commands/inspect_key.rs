...
impl InspectKeyCmd {
	...
	pub fn run(&self) -> Result<(), Error> {
		...
		if self.public {
		    ...
		    println!("expect_public_from_phrase");
...
fn expect_public_from_phrase<Pair: sp_core::Pair>(
	...
	println!("expect_public_from_phrase 3");
...
mod tests {
	...
	fn inspect_with_expected_public_key() {
		...
		let get_public_key = |seed_phrase: &str| -> (String, String) {
			...
			let pair = DilithiumPair::from_string(uri.phrase.expose_secret().as_str(), None)
				.expect("Valid pair");
		...
		let (valid_public_hex, valid_accountid) = get_public_key(seed);

		println!("checking invalid_public");
		// It should fail with the invalid public key
		check_cmd(seed, invalid_public, false);
		println!("checking valid_public_hex");

		// It should work with the valid public key & account id
		check_cmd(seed, &valid_public_hex, true);
		println!("checking valid_accountid");
		check_cmd(seed, &valid_accountid, true);

		let password = "test12245";
		let seed_with_password = format!("{}///{}", seed, password);
		let (valid_public_hex_with_password, valid_accountid_with_password) =
			get_public_key(&seed_with_password);

		println!("checking valid_public_hex with password");
		...
		println!("checking valid_accountid with password");
		check_cmd(&seed_with_password, &valid_accountid, false);

		println!("checking valid_public with password");
		...
		let (valid_public_hex_with_password_and_derivation, _) =
			get_public_key(&seed_with_password_and_derivation);
