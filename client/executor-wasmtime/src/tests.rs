...
//! Focused WAT-based coverage for the wasmtime executor adapter.
//!
//! Upstream's suite also exercises a full `sc-runtime-test` WASM blob. That crate is not
//! published, so Quantus keeps the WAT-only tests (stack limits, NaN canonicalization,
//! memory growth, precompiled artifacts, instantiation strategies) and rewrites the few
//! cases that previously depended on the unpublished blob.
...
/// No host functions — WAT fixtures used here do not import env helpers.
type HostFunctions = ();
	...
	fn build(&mut self) -> impl WasmModule + '_ {
		let blob = {
			...
			let wasm = match self.code {
				None => panic!("WAT fixture required (sc-runtime-test blob is not available)"),
				Some(ref wat) => {
...
/// WAT stand-in for upstream's `sc-runtime-test` `test_fp_f32add` export.
fn nan_canonicalization_wat() -> String {
	r#"
		(module
		  (memory $0 1)
		  (export "memory" (memory $0))
		  ...
		  (func (export "test_fp_f32add") (param $ptr i32) (param $len i32) (result i64)
		    (local $sum f32)
		    ;; SCALE-encoded ([u8;4], [u8;4]) is 8 raw little-endian bytes.
		    (local.set $sum
		      (f32.add
		    ...
		    (f32.store (local.get $ptr) (local.get $sum))
		    ;; pack_ptr_and_len(ptr, 4)
		    (i64.or
...
fn test_nan_canonicalization(instantiation_strategy: InstantiationStrategy) {
	let mut builder = RuntimeBuilder::new(instantiation_strategy)
		.use_wat(nan_canonicalization_wat())
...
fn test_rustix_version_matches_with_wasmtime() {
	...
	let wasmtime_rustix = metadata
		.packages
		.iter()
		.find(|pkg| pkg.name.as_str() == "wasmtime")
		.unwrap()
		.dependencies
		.iter()
		.find(|dep| dep.name.as_str() == "rustix")
		.unwrap();
	let our_rustix = metadata
		.packages
		.iter()
		.find(|pkg| pkg.name.as_str() == "sc-executor-wasmtime")
		.unwrap()
		.dependencies
		.iter()
		.find(|dep| dep.name.as_str() == "rustix")
