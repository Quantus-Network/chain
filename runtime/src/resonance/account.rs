...
#[cfg(feature = "std")]
impl<T: Sized + AsMut<[u8]> + AsRef<[u8]> + Public + Derive + ByteArray> Ss58Codec for T {
    fn from_string(s: &str) -> Result<Self, PublicError> {
...
pub trait Public: CryptoType + PartialEq + Eq + Clone + Send + Sync + Derive {}

/// Trait suitable for cryptographic signatures.
pub trait Signature: CryptoType + PartialEq + Eq + Clone + Send + Sync {}

/// An opaque 32-byte cryptographic identifier.
#[derive(Clone, Eq, PartialEq, Ord, PartialOrd, Encode, Decode, MaxEncodedLen, TypeInfo)]
#[cfg_attr(feature = "std", derive(Hash))]
pub struct ResonanceAccountId([u8; 32])
where
    Self: Send + Sync;

impl ResonanceAccountId {
...
impl UncheckedFrom<hash::H256> for ResonanceAccountId {
    fn unchecked_from(h: hash::H256) -> Self {
        ResonanceAccountId(h.into())
    }
}

impl ByteArray for ResonanceAccountId {
    const LEN: usize = 32;
}

#[cfg(feature = "serde")]
impl Ss58Codec for ResonanceAccountId {}

impl AsRef<[u8]> for ResonanceAccountId {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}

impl AsMut<[u8]> for ResonanceAccountId {
    fn as_mut(&mut self) -> &mut [u8] {
        &mut self.0[..]
    }
}

impl AsRef<[u8; 32]> for ResonanceAccountId {
    fn as_ref(&self) -> &[u8; 32] {
        &self.0
    }
}

impl AsMut<[u8; 32]> for ResonanceAccountId {
    fn as_mut(&mut self) -> &mut [u8; 32] {
        &mut self.0
    }
}

impl From<[u8; 32]> for ResonanceAccountId {
    fn from(x: [u8; 32]) -> Self {
        Self::new(x)
    }
}

impl<'a> TryFrom<&'a [u8]> for ResonanceAccountId {
    type Error = ();
    fn try_from(x: &'a [u8]) -> Result<ResonanceAccountId, ()> {
        if x.len() == 32 {
            let mut data = [0; 32];
            data.copy_from_slice(x);
            Ok(ResonanceAccountId(data))
        } else {
            Err(())
        }
    }
...
impl From<ResonanceAccountId> for [u8; 32] {
    fn from(x: ResonanceAccountId) -> [u8; 32] {
        x.0
    }
}

impl From<sr25519::Public> for ResonanceAccountId {
    fn from(k: sr25519::Public) -> Self {
        k.0.into()
    }
}

impl From<ed25519::Public> for ResonanceAccountId {
...
#[cfg(feature = "std")]
impl std::fmt::Display for ResonanceAccountId {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{}", self.to_ss58check())
    }
}

impl core::fmt::Debug for ResonanceAccountId {
...
#[cfg(feature = "serde")]
impl Serialize for ResonanceAccountId {
...
#[cfg(feature = "serde")]
impl<'de> Deserialize<'de> for ResonanceAccountId {
...
#[cfg(feature = "std")]
impl std::str::FromStr for ResonanceAccountId {
    type Err = &'static str;
...
/// Creates an [`ResonanceAccountId`] from the input, which should contain at least 32 bytes.
impl FromEntropy for ResonanceAccountId {
    fn from_entropy(input: &mut impl codec::Input) -> Result<Self, codec::Error> {
        Ok(ResonanceAccountId::new(FromEntropy::from_entropy(input)?))
    }
}
...
#[cfg(test)]
mod tests {
    use crate::resonance::crypto_bytes::public_bytes::PublicBytes;
    use crate::resonance::crypto_bytes::signature_bytes::SignatureBytes;
    use super::*;
...
/*        fn sign(&self, _message: &[u8]) -> Self::Signature {
    ...
    fn accountid_32_from_str_works() {
        use std::str::FromStr;
        assert!(ResonanceAccountId::from_str("5G9VdMwXvzza9pS8qE8ZHJk3CheHW9uucBn9ngW4C1gmmzpv").is_ok());
        assert!(ResonanceAccountId::from_str(
        ...
        assert_eq!(
			ResonanceAccountId::from_str("99G9VdMwXvzza9pS8qE8ZHJk3CheHW9uucBn9ngW4C1gmmzpv").unwrap_err(),
			"invalid ss58 address.",
		);
        assert_eq!(
			ResonanceAccountId::from_str(
...
//Custom traits implemented for AccountId32 outside. We have to add them here.
impl IdentifyAccount for ResonanceAccountId {
    type AccountId = Self;
