...
mod tests {
	...
	fn test_tech_collective_treasury_spend_with_root_origin() {
		TestCommons::new_test_ext().execute_with(|| {
			...
			println!("DEBUG: Waiting for referendum approval at block: {}", approval_block);
			...
			println!(
				...
				matches!(referendum_info, pallet_referenda::ReferendumInfo::Approved(_, _, _))
			);
			...
			while current_poll_block <= max_wait_block {
				if pallet_treasury::Spends::<Runtime>::get(spend_index).is_some() {
					println!("DEBUG: Treasury spend detected at block: {}", System::block_number());
					break;
				}
			...
			if let Some(_spend_info) = pallet_treasury::Spends::<Runtime>::get(spend_index) {
				println!("DEBUG: Treasury spend found at block: {}", System::block_number());
			...
			// Execute payout
			println!("DEBUG: About to attempt payout at block: {}", System::block_number());
