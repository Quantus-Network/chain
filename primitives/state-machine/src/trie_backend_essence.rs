    ...
    TrieBackendEssence<S, H, C, R>
{
    ...
    fn with_recorder_and_cache<RE>(
        ...
        let mut cache = self
            .trie_node_cache
            .as_ref()
            .map(|c| c.as_trie_db_cache(storage_root));
        let cache = cache.as_mut().map(|c| c as _);

        let mut recorder = self
            .recorder
            .as_ref()
            .map(|r| r.as_trie_recorder(storage_root));
    ...
    H::Out: Codec + Ord,
{
    ...
    fn with_trie_db<RE>(
        ...
        let db = db
            .as_ref()
            .map(|db| db as &dyn HashDBRef<H, Vec<u8>>)
            .unwrap_or(backend);
...
mod test {
    ...
    fn next_storage_key_and_next_child_storage_key_work() {
        ...
        assert_eq!(
            essence_2.next_child_storage_key(child_info, b"2"),
            Ok(Some(b"3".to_vec()))
        );
        assert_eq!(
            essence_2.next_child_storage_key(child_info, b"3"),
            Ok(Some(b"4".to_vec()))
        );
        assert_eq!(
            essence_2.next_child_storage_key(child_info, b"4"),
            Ok(Some(b"6".to_vec()))
        );
        assert_eq!(
            essence_2.next_child_storage_key(child_info, b"5"),
            Ok(Some(b"6".to_vec()))
        );
