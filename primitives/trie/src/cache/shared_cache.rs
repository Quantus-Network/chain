...
impl<H: Eq + std::hash::Hash + Clone + Copy + AsRef<[u8]>> SharedValueCache<H> {
    ...
    pub fn update(
        ...
        for hash in accessed {
            ...
            self.lru
                .get_by_hash(hash.raw(), |existing_key, _| existing_key.hash == hash);
            access_count += 1;
        }
...
impl<H: Hasher> SharedTrieCache<H> {
    ...
    pub fn peek_value_by_hash(
        ...
        self.inner
            ...
            .value_cache
            .lru
            .peek_by_hash(hash.0, |existing_key, _| {
                existing_key.is_eq(storage_root, storage_key)
            })
            .cloned()
    }
    ...
    pub(super) fn write_lock_inner(&self) -> Option<RwLockWriteGuard<'_, SharedTrieCacheInner<H>>> {
        ...
        self.inner
            .try_write_for(super::SHARED_CACHE_WRITE_LOCK_TIMEOUT)
    }
}
...
mod tests {
    ...
    fn shared_value_cache_works() {
        ...
        cache.update(
            vec![
                (
                    ValueCacheKey::new_value(&key[..], root0),
                    CachedValue::NonExisting,
                ),
                (
                    ValueCacheKey::new_value(&key[..], root1),
                    CachedValue::NonExisting,
                ),
            ],
            vec![],
        ...
        assert!(matches!(
            cache
                .lru
                .peek(&ValueCacheKey::new_value(&[1; 10][..], root0))
                .unwrap(),
        ...
        assert!(cache
            .lru
            .peek(&ValueCacheKey::new_value(&[1; 10][..], root1))
            .is_none(),);

        assert!(cache
            .lru
            .peek(&ValueCacheKey::new_value(&key[..], root0))
            .is_none());
        assert!(cache
            .lru
            .peek(&ValueCacheKey::new_value(&key[..], root1))
            .is_none());

        cache.update(
            vec![(
                ValueCacheKey::new_value(vec![10; 10], root0),
                CachedValue::NonExisting,
            )],
            vec![],
        );
