...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let _decoded_valid_delta =
		Vec::<(Vec<u8>, Option<Vec<u8>>)>::decode(&mut &valid_delta_encoded[..])?;
	let _decoded_invalid_delta =
		Vec::<(Vec<u8>, Option<Vec<u8>>)>::decode(&mut &invalid_delta_encoded[..])?;
