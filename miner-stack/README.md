...
On first start the node writes `node-data/chains/<chain>/miner-auth-token`
(not logged — read the file) and `miner-tls-cert-sha256` (logged). The bundled
miner reads them via `--auth-token-file` / `--tls-cert-sha256-file` (Compose
mounts `./node-data` read-only). If the miner starts before the files exist it
will exit and restart until they appear.
...
> node's TLS cert fingerprint (auto-written under `node-data/chains/<chain>/`;
> fingerprint is logged, token is not), but you should still keep the port off
> the public internet.
>
