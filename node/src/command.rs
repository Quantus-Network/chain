...
use qp_dilithium_crypto::{traits::WormholeAddress, Dilithium65Pair, Dilithium87Pair};
use qp_rusty_crystals_hdwallet::{
	derive_wormhole_from_mnemonic, generate_mnemonic, mnemonic_to_seed, wormhole::WormholePair,
	SensitiveBytes32, SensitiveBytes64, QUANTUS_DILITHIUM_CHAIN_ID, QUANTUS_WORMHOLE_CHAIN_ID,
};
...
fn default_hd_path(scheme: QuantusAddressType, index: u32) -> String {
	let chain_id = match scheme {
		QuantusAddressType::Wormhole => QUANTUS_WORMHOLE_CHAIN_ID,
		QuantusAddressType::MlDsa87 | QuantusAddressType::MlDsa65 => QUANTUS_DILITHIUM_CHAIN_ID,
	};
	format!("m/44'/{chain_id}/{index}'/0'/0'")
}

fn dilithium_account_details(
	public: impl AsRef<[u8]> + Into<AccountId32>,
	secret: &[u8],
	seed_hex: String,
	secret_phrase: Option<String>,
) -> QuantusKeyDetails {
	let public_key_hex = format!("0x{}", hex::encode(public.as_ref()));
	let account_id: AccountId32 = public.into();
	QuantusKeyDetails {
		address: account_id.to_ss58check_with_version(Ss58AddressFormat::custom(189)),
		raw_address: format!("0x{}", hex::encode(account_id)),
		public_key_hex,
		secret_key_hex: format!("0x{}", hex::encode(secret)),
		seed_hex,
		secret_phrase,
		inner_hash: None,
	}
...
#[allow(clippy::result_large_err)]
fn derive_dilithium_from_mnemonic(
	...
	let derived = "N/A (derived from mnemonic)".to_string();
	match scheme {
		QuantusAddressType::MlDsa87 => {
			let keypair = qp_rusty_crystals_hdwallet::ml_dsa_87::derive_key_from_mnemonic(
				mnemonic, None, path,
			)
			.map_err(|e| {
			...
			let pair = Dilithium87Pair::from_keypair(keypair);
			Ok(dilithium_account_details(
				pair.public(),
				pair.secret_bytes(),
				derived,
				secret_phrase,
		...
		QuantusAddressType::MlDsa65 => {
			let keypair = qp_rusty_crystals_hdwallet::ml_dsa_65::derive_key_from_mnemonic(
				mnemonic, None, path,
			)
			.map_err(|e| {
				eprintln!("Error deriving from mnemonic: {:?}", e);
			...
			let pair = Dilithium65Pair::from_keypair(keypair);
			Ok(dilithium_account_details(
				pair.public(),
				pair.secret_bytes(),
				derived,
		...
		QuantusAddressType::Wormhole => unreachable!("wormhole is not a dilithium scheme"),
	}
}

#[allow(clippy::result_large_err)]
fn dilithium_from_seed(
	scheme: QuantusAddressType,
	seed_for_pair: Zeroizing<Vec<u8>>,
	secret_phrase: Option<String>,
) -> Result<QuantusKeyDetails, sc_cli::Error> {
	let seed_hex = format!("0x{}", hex::encode(&seed_for_pair));
	let details = match scheme {
		QuantusAddressType::MlDsa87 => {
			let pair = Dilithium87Pair::from_seed(&seed_for_pair).map_err(|e| {
				eprintln!("Error creating Dilithium87Pair: {:?}", e);
				sc_cli::Error::Input("Failed to create keypair".into())
			})?;
			dilithium_account_details(pair.public(), pair.secret_bytes(), seed_hex, secret_phrase)
		},
		QuantusAddressType::MlDsa65 => {
			let pair = Dilithium65Pair::from_seed(&seed_for_pair).map_err(|e| {
				eprintln!("Error creating Dilithium65Pair: {:?}", e);
				sc_cli::Error::Input("Failed to create keypair".into())
			})?;
			dilithium_account_details(pair.public(), pair.secret_bytes(), seed_hex, secret_phrase)
		},
...
#[allow(clippy::result_large_err)]
fn mnemonic_to_pair_seed(mnemonic: String) -> Result<Zeroizing<Vec<u8>>, sc_cli::Error> {
	let mut seed64 = SensitiveBytes64::zeroed();
	mnemonic_to_seed(mnemonic, None, &mut seed64).map_err(|e| {
		eprintln!("Error processing provided words: {:?}", e);
		sc_cli::Error::Input("Failed to process provided words".into())
	})?;
	Ok(Zeroizing::new(seed64.as_bytes().to_vec()))
}

#[allow(clippy::result_large_err)]
fn generate_new_mnemonic() -> Result<Zeroizing<String>, sc_cli::Error> {
	let mut entropy = [0u8; 32];
	rand::thread_rng().fill(&mut entropy);
	let sensitive_entropy = SensitiveBytes32::from(&mut entropy);
	generate_mnemonic(sensitive_entropy).map_err(|e| {
...
pub fn generate_quantus_key(
	...
	match scheme {
		QuantusAddressType::MlDsa87 | QuantusAddressType::MlDsa65 => {
			...
			if let Some(ref words_phrase) = words {
				...
				if let Some(path) = derivation_path {
					return derive_dilithium_from_mnemonic(
						scheme,
						words_phrase,
						path,
						words_to_print,
				...
				seed_for_pair = mnemonic_to_pair_seed(words_phrase.to_string())?;
				...
				let new_words = generate_new_mnemonic()?;
				words_to_print = Some(new_words.to_string());

				if let Some(path) = derivation_path {
					let details =
						derive_dilithium_from_mnemonic(scheme, &new_words, path, words_to_print)?;
					drop(new_words);
					return Ok(details);
				}
				seed_for_pair = mnemonic_to_pair_seed(new_words.to_string())?;
			};

			dilithium_from_seed(scheme, seed_for_pair, words_to_print)
		},
		QuantusAddressType::Wormhole => {
			...
			let wormhole_pair = if let Some(path) = derivation_path {
		...
		Some(Subcommand::Key(cmd)) => {
			match cmd {
				...
				QuantusKeySubcommand::Quantus {
					...
					if *seed && derivation_path.is_some() {
						return Err(sc_cli::Error::Input(
							"--derivation-path cannot be used with --seed".into(),
						));
					}
					let resolved_index = if derivation_path.is_some() || *no_derivation || *seed {
						None
					} else {
						Some(wallet_index.unwrap_or(0))
					};
					let resolved_path: Option<String> =
						if let Some(path) = derivation_path.as_deref() {
							Some(path.to_string())
						} else if let Some(index) = resolved_index {
							Some(default_hd_path(*scheme, index))
						} else {
							None
						};
					match generate_quantus_key(
						*scheme,
						...
						Ok(details) => {
							match scheme {
								QuantusAddressType::MlDsa87 | QuantusAddressType::MlDsa65 => {
									println!("Generating Quantus {scheme} address...");
									if *seed {
...
mod tests {
	...
	const DILITHIUM_HD_0: &str = "m/44'/189189'/0'/0'/0'";
	const DILITHIUM_HD_1: &str = "m/44'/189189'/1'/0'/0'";
	...
	fn provided_mnemonic_is_not_echoed_back() {
		...
		let standard =
			generate_quantus_key(QuantusAddressType::MlDsa87, None, Some(mnemonic.clone()), None)
				.unwrap();
		assert!(
		...
		let wormhole_path = default_hd_path(QuantusAddressType::Wormhole, 0);
		let wormhole = generate_quantus_key(
			QuantusAddressType::Wormhole,
			None,
			Some(mnemonic),
			Some(&wormhole_path),
	...
	fn test_generate_quantus_key_standard_new_mnemonic() {
		...
		let result =
			generate_quantus_key(QuantusAddressType::MlDsa87, None, None, Some(DILITHIUM_HD_0));
		assert!(result.is_ok());
	...
	fn test_generate_quantus_key_wormhole() {
		...
		let wormhole_path = default_hd_path(QuantusAddressType::Wormhole, 0);
		let result =
			generate_quantus_key(QuantusAddressType::Wormhole, None, None, Some(&wormhole_path));
		assert!(result.is_ok());
		let details = result.unwrap();
	...
	fn test_generate_quantus_key_invalid_seed_length() {
		...
		let result = generate_quantus_key(QuantusAddressType::MlDsa87, seed, None, None);
	...
	#[test]
	fn test_generate_quantus_key_ml_dsa_65_differs_from_87_at_same_path() {
		let mnemonic = TEST_MNEMONIC.to_string();
		let pair87 = generate_quantus_key(
			QuantusAddressType::MlDsa87,
			None,
			Some(mnemonic.clone()),
			Some(DILITHIUM_HD_0),
		...
		.unwrap();
		let pair65 = generate_quantus_key(
			QuantusAddressType::MlDsa65,
			None,
			Some(mnemonic),
			Some(DILITHIUM_HD_0),
		...
		.unwrap();
		assert_eq!(pair87.address, TEST_ADDRESS_HD_0);
		assert_ne!(pair65.address, pair87.address);
		assert!(
			AccountId32::from_ss58check_with_version(&pair65.address).is_ok(),
			"ML-DSA-65 address should be valid SS58: {}",
			pair65.address
		);
	}

	#[test]
	fn key_quantus_parses_scheme_and_derivation_path() {
		use clap::Parser;
		...
		.expect("parse ml-dsa-65 with derivation path");
		match cli.subcommand {
			Some(Subcommand::Key(QuantusKeySubcommand::Quantus {
				scheme,
				derivation_path,
				wallet_index,
				..
			})) => {
				assert_eq!(scheme, QuantusAddressType::MlDsa65);
				assert_eq!(derivation_path.as_deref(), Some(DILITHIUM_HD_1));
				assert_eq!(wallet_index, None);
			},
		...
		.expect("parse --scheme standard alias");
		match cli.subcommand {
			Some(Subcommand::Key(QuantusKeySubcommand::Quantus { scheme, .. })) => {
				assert_eq!(scheme, QuantusAddressType::MlDsa87);
			},
			other => panic!("unexpected subcommand: {other:?}"),
		}
		...
		assert!(
			...
			.is_err(),
			"--derivation-path and --wallet-index must conflict"
		);
	}
