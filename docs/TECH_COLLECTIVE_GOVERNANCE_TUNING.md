...
Removal does not eagerly reconcile ongoing tallies (that path was an unbounded `Voting` scan). Stale votes of a removed (or otherwise ineligible) member are withdrawn permissionlessly via `remove_ineligible_vote`; completed-poll records via `cleanup_poll`. Until then the support denominator `MemberCount[0]` has already shrunk, so enacting proposals should batch `remove_ineligible_vote` with the removal.
