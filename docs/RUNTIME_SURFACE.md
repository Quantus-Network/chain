...
- **Payout quantization:** wormhole leaves commit `amount / 10^10`, so a sub-quantum payout would create a zero-value leaf and strand funds on a keyless beneficiary. Totals must be multiples of `PayoutQuantum`; every payout is rounded down to a multiple and `claimed` advances only by the paid amount (stays aligned, final claim at `end` is exact). `end_schedule` sends sub-quantum vested dust to the treasury (signature-controlled, needs no leaf).
