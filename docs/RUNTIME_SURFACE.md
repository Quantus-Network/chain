...
- `MaxSigners = 100`, `MaxTotalProposalsInStorage = 200`, `MaxCallSize = 10 KB`, `MultisigFee = 0.6 UNIT` (burned), `ProposalDeposit = 1 UNIT`, `ProposalFee = 1 UNIT`, `MaxExpiryDuration ≈ 2 weeks`, `MaxInnerCallWeight = (10^12, 2.5 MB)`, `HighSecurity = HighSecurityConfig`, `StoredCallFilter = StoredMultisigCallFilter`, `PalletId = "py/mltsg"`.
...
- **Stored-call filter:** `propose` checks the decoded call against the runtime's `StoredCallFilter` (`StoredMultisigCallFilter`: statically counted reaps ≤ 32 and recordable transfers ≤ `STORED_CALL_COUNT_CAP = 32`), and `execute` re-checks it (`StoredCallNotAllowed`). This bounds the side effects a stored call can hide from pre-dispatch admission, letting the transaction extensions reserve flat per-`execute` weights (see §3 weight accounting and TxExtension items 7/9); proposals needing more transfers/reaps must be split.
