...
- **Weight accounting:**
  ...
  - Reap cleanup (1 DB write for `Pubkeys::remove`): registered by the `OnKilledAccount` hook itself via `frame_system::register_extra_weight_unchecked`, at the moment of the write — this covers every reap path (user-signed, scheduled, root, or from pallets added later) by construction, rather than depending on a hand-maintained list of kill-capable call weights. A first-registration that also reaps therefore pays verify (R+W) via the signed extension plus cleanup (W) as registered block weight.
...
7. `(transaction_extensions::ChargePubkeyCacheVerify, frame_system::CheckWeight)` — nested pair (the `TransactionExtension` tuple impl caps at 12 elements; nesting is wire- and metadata-transparent). `ChargePubkeyCacheVerify` is **custom** and zero-sized: it reserves `PubkeyCacheVerifyWeight` as `extension_weight` on signed extrinsics for the pubkey-cache DB work done during signature verification; bare unsigned extrinsics report `extension_weight = 0` and pay nothing.
