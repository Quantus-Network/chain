...
fn genesis_config_works() {
	new_test_ext().execute_with(|| {
		...
		let signatories = TreasuryMultisig::signatories();
		assert_eq!(signatories.len(), 5);
		assert_eq!(signatories.to_vec(), vec![1, 2, 3, 4, 5]);

		let threshold = TreasuryMultisig::threshold();
...
fn set_treasury_signatories_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(TreasuryMultisig::set_treasury_signatories(
			RuntimeOrigin::root(),
		...
		assert_eq!(TreasuryMultisig::signatories().to_vec(), new_signatories);
		assert_eq!(TreasuryMultisig::threshold(), 3);
...
fn set_treasury_signatories_validates_empty() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			TreasuryMultisig::set_treasury_signatories(RuntimeOrigin::root(), vec![], 1),
...
fn set_treasury_signatories_validates_threshold() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			TreasuryMultisig::set_treasury_signatories(RuntimeOrigin::root(), vec![1, 2, 3], 0),
			Error::<Test>::InvalidThreshold
		);

		// Threshold > signatories should fail
		assert_noop!(
			TreasuryMultisig::set_treasury_signatories(RuntimeOrigin::root(), vec![1, 2, 3], 4),
			Error::<Test>::InvalidThreshold
		);
	});
}
...
fn set_treasury_signatories_validates_max() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			TreasuryMultisig::set_treasury_signatories(RuntimeOrigin::root(), too_many, 50),
...
fn changing_threshold_changes_address() {
	new_test_ext().execute_with(|| {
		...
		let account_threshold_4 = TreasuryMultisig::get_treasury_account();
...
fn deterministic_address_generation() {
	new_test_ext().execute_with(|| {
		...
		let account2 = TreasuryMultisig::get_treasury_account();
