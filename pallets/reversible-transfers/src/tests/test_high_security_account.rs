...
// NOTE: Many of these behaviors are enforced via SignedExtension or external pallets
// (Recovery/Proxy). Where runtime support is not yet wired in the mock, tests are marked #[ignore]
// with rationale.
...
fn immediate_transfers_are_converted_to_reversible_with_configured_delay() {
	new_test_ext().execute_with(|| {
		...
		// The concrete assertion will require extracting the generated tx_id or scanning
		// PendingTransfers. Placeholder assert until extension behavior is wired in the mock.
...
fn guardian_can_add_inheritance_recovery_with_single_beneficiary_and_six_month_delay() {
	new_test_ext().execute_with(|| {
		...
		// Placeholder: guardian initiates recovery on behalf of hs_user via proxy or recovery
		// pallet API. assert_ok!(Recovery::create_recovery(...));
