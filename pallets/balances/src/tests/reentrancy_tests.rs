...
fn transfer_dust_removal_tst1_should_work() {
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			assert_ok!(Balances::force_set_balance(RawOrigin::Root.into(), account_id(1), 1000));
			assert_ok!(Balances::force_set_balance(RawOrigin::Root.into(), account_id(2), 500));
			...
			assert_ok!(Balances::transfer_allow_death(
				RawOrigin::Signed(account_id(2)).into(),
				account_id(3),
				450
			));
			...
			assert_eq!(Balances::free_balance(account_id(3)), 450);
...
fn emit_events_with_no_existential_deposit_suicide_with_dust() {
	ExtBuilder::default().existential_deposit(2).build_and_execute_with(|| {
		...
		let res = Balances::withdraw(&account_id(1), 98, BestEffort, Protect, Force);
		assert_eq!(res.unwrap().peek(), 98);

		// no events
		assert_eq!(
			events(),
			[RuntimeEvent::Balances(crate::Event::Withdraw { who: account_id(1), amount: 98 })]
		);

		let res = Balances::withdraw(&account_id(1), 1, BestEffort, Expendable, Force);
		...
		assert_eq!(
			...
			[
				...
				RuntimeEvent::Balances(crate::Event::Withdraw { who: account_id(1), amount: 1 }),
			]
		);
	});
}
