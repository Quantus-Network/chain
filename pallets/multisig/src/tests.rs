...
#[test]
fn migration_v0_to_v1_drops_call_weight() {
	use crate::migrations::{v1::OldProposalData, MigrateV0ToV1};
	use frame_support::traits::{GetStorageVersion, OnRuntimeUpgrade, StorageVersion};

	new_test_ext().execute_with(|| {
		// Pretend the pallet is still at on-chain storage version 0.
		StorageVersion::new(0).put::<crate::Pallet<Test>>();

		let addr = AccountId32::new([9u8; 32]);
		let proposer = AccountId32::new([1u8; 32]);
		let id = 0u32;
		let call: crate::BoundedCallOf<Test> = vec![1u8, 2, 3].try_into().unwrap();
		let approvals: crate::BoundedApprovalsOf<Test> = vec![proposer.clone()].try_into().unwrap();
		let expiry: u64 = 100;
		let deposit: u128 = 1_000;

		// Write a record in the v0 layout (with call_weight) at the real Proposals key.
		let old = OldProposalData {
			proposer: proposer.clone(),
			call: call.clone(),
			call_weight: Weight::from_parts(123, 456),
			expiry,
			approvals: approvals.clone(),
			deposit,
			status: ProposalStatus::Active,
		};
		let key = Proposals::<Test>::hashed_key_for(&addr, id);
		frame_support::storage::unhashed::put_raw(&key, &old.encode());

		MigrateV0ToV1::<Test>::on_runtime_upgrade();

		let migrated = Proposals::<Test>::get(&addr, id).expect("record migrated to v1 layout");
		assert_eq!(migrated.proposer, proposer);
		assert_eq!(migrated.call, call);
		assert_eq!(migrated.expiry, expiry);
		assert_eq!(migrated.approvals, approvals);
		assert_eq!(migrated.deposit, deposit);
