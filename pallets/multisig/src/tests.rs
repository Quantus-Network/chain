...
fn dissolve_multisig_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Multisig::approve_dissolve(
			RuntimeOrigin::signed(bob()),
			multisig_address.clone(),
			bob() // sweep_beneficiary (unused since threshold not reached)
		));
...
#[test]
fn dissolve_multisig_sweeps_dust_balance() {
	new_test_ext().execute_with(|| {
		System::set_block_number(1);
		...
		// Simulate griefing: external party sends dust to multisig
		let griefer = dave();
		...
		assert_eq!(Balances::free_balance(&multisig_address), dust_amount);

		// Track beneficiary balance before dissolution
		let beneficiary = bob();
		let bob_balance_before = Balances::free_balance(&beneficiary);
		...
		// Still exists (threshold not reached), dust still there
		assert!(Multisigs::<Test>::contains_key(&multisig_address));
		assert_eq!(Balances::free_balance(&multisig_address), dust_amount);

		// Approve dissolve by Charlie (2nd approval - threshold reached!)
		// Dust should be swept to beneficiary
		assert_ok!(Multisig::approve_dissolve(
			RuntimeOrigin::signed(charlie()),
			multisig_address.clone(),
			beneficiary.clone()
		...
		// Dust was swept to beneficiary
		assert_eq!(
			Balances::free_balance(&beneficiary),
			bob_balance_before + dust_amount
		);

		// Multisig account balance is now zero
		assert_eq!(Balances::free_balance(&multisig_address), 0);

		// Creator got deposit back
...
fn dissolve_multisig_fails_with_proposals() {
	new_test_ext().execute_with(|| {
		...
		assert_err_ignore_postinfo(
			Multisig::approve_dissolve(RuntimeOrigin::signed(bob()), multisig_address.clone(), bob()),
