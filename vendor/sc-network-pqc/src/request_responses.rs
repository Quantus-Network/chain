...
impl RequestResponsesBehaviour {
    ...
    pub fn new(
        ...
        for protocol in list {
			log::info!(
				target: "sub-libp2p",
				"Registering request-response protocol: {:?} with timeout {:?}",
				protocol.name,
				protocol.request_timeout,
			);
            let mut cfg = Config::default();
            cfg.set_request_timeout(protocol.request_timeout);
			// cfg.set_connection_keep_alive(protocol.request_timeout); // deprecated, we use swarm setting for this
            log::info!(target: "sub-libp2p", "req-resp registered: protocol={:?}, timeout={:?}", protocol.name, protocol.request_timeout);
            ...
            if self.periodic_request_check.poll_tick(cx).is_ready() {
                self.pending_requests.retain(|id, req| {
					...
					if elapsed > *request_timeout {
						log::warn!(
							target: "sub-libp2p",
							"Periodic checker TIMEOUT for {:?}. Elapsed: {:?}, Configured: {:?}",
							id,
							elapsed,
							request_timeout,
						);
                        ...
                        log::debug!(target: "sub-libp2p", "Request timeout: protocol={:?}, elapsed={:?}, limit={:?}", id.protocol, elapsed, request_timeout);
						if let Some(response_tx) = req.response_tx.take() {
                    ...
                    match ev {
                        ...
                        request_response::Event::OutboundFailure {
							...
							if matches!(error, request_response::OutboundFailure::Timeout) {
								log::warn!(
									target: "sub-libp2p",
									"Libp2p TIMEOUT for request {:?} on protocol {:?}",
									request_id,
									protocol,
								);
							}
...
mod tests {
    ...
    async fn basic_request_response_works() {
        ...
        let (mut swarm0, _) = swarms.remove(0);
        // Running `swarm[0]` in the background.
        tokio::spawn(async move {
            loop {
                match swarm0.select_next_some().await {
        ...
        let (mut swarm1, _) = swarms.remove(0);
        ...
        loop {
            match swarm1.select_next_some().await {
                SwarmEvent::ConnectionEstablished { peer_id, .. } => {
                    ...
                    swarm1.behaviour_mut().send_request(
                        &peer_id,
                        protocol_name.clone(),
    ...
    async fn request_fallback() {
        ...
        assert_eq!(
            response_receiver.await.unwrap().unwrap(),
            (
                b"this is a response on protocol /test/other".to_vec(),
                protocol_name_2.clone()
            )
        );
    }
